#ifndef __MYSQLDATABASE_HPP__
#define __MYSQLDATABASE_HPP__
        ...
        uint32 ConnectionCount, uint32 BufferSize)
    {
        ...
        for( i = 0; i < ConnectionCount; ++i )
        {
            ...
            if(mysql_options(temp, MYSQL_SET_CHARSET_NAME, "utf8"))
                perror("MySQLDatabase: Could not set utf8 character set.");

            if (mysql_options(temp, MYSQL_OPT_RECONNECT, &my_true))
                perror("MySQLDatabase: MYSQL_OPT_RECONNECT could not be set, connection drops may occur but will be counteracted.");
...
protected:
    ...
    bool _SendQuery(DatabaseConnection *con, const char* Sql, bool Self = false, bool bSave = true)
    {
	...
	void _BeginTransaction(DatabaseConnection * conn) { _SendQuery( conn, "START TRANSACTION", false ); }
	void _EndTransaction(DatabaseConnection * conn) { _SendQuery( conn, "COMMIT", false ); }
