...
//		connect(this, SIGNAL(updateGraphOptions()), m, SIGNAL(updateGraphOptionsSig()));
		...
		connect(this, SIGNAL(highLightGeoComponent(Geometry::GeoComponent*)), _mainWindow, SIGNAL(highLightGeoComponentSig(Geometry::GeoComponent*)));
...
//			item->setCheckState(0, isvisable);
	...
	void GeometryTreeWidget::singleClicked(QTreeWidgetItem* item, int i)
	{
		...
		case GeoComponentChild:
		{
			...
			emit highLightGeoComponent(aGc);
			break;
		}
	...
	void GeometryTreeWidget::removeData()
	{
		...
		if (index < 0) return;
		auto geoset = _data->getGeometrySetAt(index);
		int geoid = geoset->getID();
		QString code = QString("MainWindow.deleteGeometry(%1)").arg(geoid);
