...
namespace MeshData
{
	...
	MshReader::MshReader(const QString filename)
	{
		_fileName = filename;
	...
	void MshReader::readPoints10(vtkUnstructuredGrid* dataset, QString info)
	{
...
//			set->setIDList(idlist);
	...
	void MshReader::readPoints130(vtkUnstructuredGrid* dataset, QString info)
	{
		vtkSmartPointer<vtkPoints> points = dataset->GetPoints();
		if (points == nullptr)
		{
...
//		set->setIDList(idlist);
	...
	void MshReader::readCells120(vtkUnstructuredGrid* dataset, QString info)
	{
		QStringList infos = info.split(" ");
		const int setid = infos.at(1).toInt();
...
//		set->setIDList(idlist);
	...
	void MshReader::readFluentCells12(vtkUnstructuredGrid* dataset, QString info)
	{
		QStringList infos = info.split(" ");
...
//		set->setIDList(idlist);
	...
	void MshReader::readZone45(QString zone)
	{
//		qDebug() << zone;
	...
	bool MshReader::readFace13(vtkUnstructuredGrid* dataset,QString info)
	{
		int index;
...
//		set->setIDList(idlist);
	...
	QString MshReader::getNextLineInfo(QString line)
	{
		QString nextLine;
	...
	bool MshReader::isHex(QString line)
	{
		bool isHex = false;
	...
	int MshReader::strToInt(QString line, bool isHex)
	{
		if (isHex)
		{
	...
	bool MshReader::readGambitFile(vtkUnstructuredGrid* dataset)
	{
		QString line;
		do
		{
...
//				set->setDataSet(dataset);
	...
	bool MshReader::readFluentFile(vtkUnstructuredGrid* dataset)
	{
		QString line;
		do
		{
