...
namespace Plugins
{
	...
	private slots:
		...
		void updateFluidAction(int status);
		void updateFilterMeshAction(int status);
		void updateMeshModelingAction(int status);
	...
	private:
		...
		QAction* _fluidMesh{};                                  //网格菜单--fluid
		QAction* _filterMesh{};                                  //网格菜单--filter
		QAction* _meshModeling{};									//网格菜单--网格建模
		...
		FastCAEDesigner::QFWidgetAction *_meshFluidCheckbox;
		FastCAEDesigner::QFWidgetAction *_meshFilterCheckbox;
		...
		QToolBar* _geometryEditToolBar;			//几何编辑
		QToolBar* _measuredDistanceToolBar;		//测量距离
