...
namespace GUI
{
	...
	void MainWindow::registerMoudel()
	{
		...
		this->tabifyDockWidget(_messageWindow,_processWindow);
		this->setTabPosition(Qt::BottomDockWidgetArea, QTabWidget::TabPosition::North);
		_messageWindow->raise();
	}
	...
	void MainWindow::setSketchPageVisible(bool visible)
	{
		...
		if (ribbon->getCurrentIndex() == 0)    return;
	...
	void MainWindow::changeStyleToRibbon(bool ok)
	{
		...
		if (isRibbon == ok)  return;
		QMessageBox mbox(QMessageBox::Information, tr("Info"), tr("Restart to load the style!"));
		QPushButton *button1 = mbox.addButton(tr("Restart later"), QMessageBox::RejectRole);
	...
	QPair<QWidget*, QList<QAction*>> MainWindow::createWidgetAndAction(const QString& title, const QStringList& actionStrs)
	{
		...
		if (isUseRibbon())
		{
			...
			SARibbonPannel* pannel = page->addPannel(QString());
