...
#include "GeometryCommand/GeoCommandCreateComponent.h"
namespace GUI
{
...
//		_pyAgent->unLock();
	...
	void MainWindowPy::deleteGeometry(int id)
	{
		MainWidget::PreWindow* pre = _subWindowManager->getPreWindow();
		if (id < 0) return; 
		auto geo = Geometry::GeometryData::getInstance()->getGeometrySetByID(id);
		Geometry::GeometryData::getInstance()->removeTopGeometrySet(geo);
		emit pre->removeGemoActors(geo);
		emit _mainWindow->updateGeometryTreeSig();
		emit _mainWindow->updateActionStatesSig();
		emit _mainWindow->updateGraphOptionsSig();
		_pyAgent->unLock();
	}
	...
	void MainWindowPy::createGeoComponent(char* name, char* type, char* strgIDs, char* striIDs)
	{
		QString sname(name);
		QString stype(type);
		QStringList geoIds = QString(strgIDs).split(';');
		QStringList itemIds = QString(striIDs).split(';');
		if (geoIds.size() != itemIds.size())	return;
		Command::CommandCreateComponent* command = new Command::CommandCreateComponent(_mainWindow, _subWindowManager->getPreWindow());
		command->setName(sname);
		command->setType(stype);
		command->setMembers(geoIds, itemIds);
