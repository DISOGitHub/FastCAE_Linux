...
namespace Plugins
{
	...
	void PHengLEIWriter::startSolver(ModelData::ModelDataBase* model,QString process, bool Mpi)
	{
		...
		if (solver == nullptr)
		{
			QMessageBox::information(nullptr, tr("Title"), tr("Please set up the solver first!"));
			return;
		}
		auto m = Plugins::PHengLEIPlugin::getMainWindow();
		if (model==nullptr)
		{
			QMessageBox::information(nullptr, tr("Title"), tr("Please tra Mesh!"));
			return;
		}
...
*/
	...
	void PHengLEIWriter::writeKeyFile(ModelData::ModelDataBase* model)
	{
		const QString path = model->getPath();
		{
			...
			QFile kfile(keyfile);
			if (!kfile.open(QIODevice::WriteOnly)) 
			{
				QMessageBox::information(nullptr, QObject::tr("Title"), QObject::tr("key.hypara doesn't exist!"));
				return;
			}
	...
	void PHengLEIWriter::writeGridPara(ModelData::ModelDataBase* model)
	{
		...
		{
			QMessageBox::information(nullptr, QObject::tr("Title"), QObject::tr("grid_para.hypara doesn't exist!"));
			return;
		}
	...
	QString PHengLEIWriter::readPhengLeiFile(ModelData::ModelDataBase* model, QString filename)
	{
		...
		{
			QMessageBox::information(NULL, tr("Title"), tr("%1 don't exist!").arg(keyfile));
			return QString();
		}
		readFile.close();
		return strAll;
