...
namespace Plugins
{
	...
	bool PHengLEIPlugin::uninstall()
	{
		...
		ProjectTree::ProjectTreeConfig::removeContextMenu(1, "Transform", "MeshSetting");
		ProjectTree::ProjectTreeConfig::removeContextMenu(1, "Partition", "MeshSetting");

		ProjectTree::ProjectTreeConfig::removeContextMenu(2, "Transform", "MeshSetting");
		ProjectTree::ProjectTreeConfig::removeContextMenu(2, "Partition", "MeshSetting");

		ProjectTree::ProjectTreeConfig::removeContextMenu(3, "Transform", "MeshSetting");
		ProjectTree::ProjectTreeConfig::removeContextMenu(3, "Partition", "MeshSetting");
		return true;
	}
...
void PHENGLEIPLUGINAPI MeshTransfer(ModelData::ModelDataBase* model, ProjectTree::ProjectTreeBase* tree)
{
	...
	if (solver == nullptr)
	{
		QMessageBox::information(nullptr, QObject::tr("Title"), QObject::tr("Please set up the solver first!"));
		return;
	}
	if (MeshData::MeshData::getInstance()->getKernalCount() < 1)
	{
		QMessageBox::information(NULL, QObject::tr("Title"), QObject::tr("Please import the grid first!"));
		return;
	}
	//设置求解器路径
	Plugins::PHengLEIWriter* writer;
	...
	if (!result) return;
	const QString exepath = model->getPath() + "/PHengLEIv3d0.exe";
	solver->setEXEPath(exepath);
...
void  ParameterSetting(ModelData::ModelDataBase* model)
{
	...
	if (!parfile.open(QIODevice::WriteOnly)) 
	{
		QMessageBox::information(nullptr, QObject::tr("Title"), QObject::tr("%1.hypara doesn't exist!").arg(casename));
		return;
	}
...
void GetBoundaryCondition(ModelData::ModelDataBase* data)
{
	...
	QFile bcFile(bcPath);
	if (!bcFile.open(QIODevice::Text | QIODevice::ReadWrite | QIODevice::Truncate))
	{
		QMessageBox::information(nullptr, QObject::tr("Title"), QObject::tr("boundary_condition.hypara doesn't exist!"));
		return;
	}
...
void ParallelPartition(ModelData::ModelDataBase* model)
{
	...
	if (!parfile.open(QIODevice::WriteOnly)) 
	{
		QMessageBox::information(nullptr, QObject::tr("Title"), QObject::tr("partition.hypara doesn't exist!"));
		return;
	}
	QTextStream stream(&parfile);
...
bool checkCalFiles(ModelData::ModelDataBase* model)
{
	if (model == nullptr) return false;
	Plugins::PHengLEIPlugin* plug = Plugins::PHengLEIPlugin::getSelf();
	QString casename = plug->getPhengLeiCase();
	QString filename{};
	if (casename == "Subsonic")  filename = "cfd_para_subsonic.hypara";
	else if (casename == "Transonic") filename = "cfd_para_transonic.hypara";
	else filename = "cfd_para_hypersonic.hypara";

	const QString path = model->getPath();
	if ((QFile(path + "/bin/key.hypara").exists()) &&
		(QFile(path + "/bin/cfd_para.hypara").exists()) &&
		(QFile(path+"/bin/"+filename).exists()))
	{
		return true ;
	}
	else
